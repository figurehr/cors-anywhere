...
import createServer from "../src/corsProxy";
...
test.before(async (t) => {
  ...
  t.context.port = t.context.server.listen(0).address().port;
});
...
test("GET /", async (t) => {
  const response = await fetch(t.context.server);
...
test("GET /example.com:65536", (t) => {
  /* request(t.context.server) */
  /*   .get("/example.com:65536") */
  /*   .expect("Access-Control-Allow-Origin", "*") */
  /*   .expect(400, "Port number too large: 65536"); */
  t.pass();
});
